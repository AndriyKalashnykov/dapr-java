...
Last reviewed: 2026-06-15 (re-verified against Maven Central + Docker Hub: project is on dapr-sdk `1.17.2` GA, and `1.17.3` has no GA on Maven Central yet — still pre-release; `kindest/node:v1.36.1` is now PUBLISHED (Docker Hub, ~2026-06-02), superseding the prior "no v1.36.x tag yet" note — the K8s 1.36 bump is now unblocked, see below).
...
- [ ] **`dapr-spring-boot-4-starter` 1.17.3 GA on Maven Central** — project is currently on `1.17.2` GA (`<dapr.version>` in pom.xml); `1.17.3` has not reached GA on Maven Central (re-verified 2026-06-15). Bump `dapr.version` (and `testcontainers-dapr.version`, since they're the same property) when 1.17.3 GA lands. Runtime/Helm chart is already on 1.17.7 — runtime-ahead-of-SDK is normal Dapr Java cadence.
...
- [ ] **kindest/node v1.36 + kubectl 1.36** — Kubernetes 1.36.0 GA'd 2026-05-07. `kindest/node:v1.36.1` is now PUBLISHED on Docker Hub (re-verified 2026-06-15) — the node image has landed, so this is unblocked. Bump the node image to `v1.36.1` and `kubectl` from 1.35.4 to 1.36.x together (keep cluster ↔ kubectl skew at +1 minor max), and re-run `make e2e` to validate the KinD stack against the new node image before committing.
