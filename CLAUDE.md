...
Last reviewed: 2026-06-15 (re-verified against Maven Central + Docker Hub: project is on dapr-sdk `1.17.2` GA, and `1.17.3` has no GA on Maven Central yet — still pre-release; the K8s 1.36 bump LANDED this session — KinD 0.32.0 + kindest/node v1.36.1 + kubectl 1.36.2, see the checked-off backlog item below).
