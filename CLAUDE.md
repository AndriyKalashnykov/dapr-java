...
make nvd-refresh                        # Refresh the NVD DB only (update-only — no scan, CANNOT fail on a CVE). What the MANUAL nvd-cache-refresh CI job runs; dispatch it before tagging to make cve-check fast
...
| `e2e-shared`, `e2e-prod-backends` | **weekly cron** + dispatch | The only coverage of the alternate topologies; nothing else exercises them. |
| `nvd-cache-refresh` | **`workflow_dispatch` ONLY** | Warms the NVD cache. Deliberately OFF the cron (owner opted out of the weekly spend), so it only runs when you ask for it. `continue-on-error`, gates nothing. See "NVD cache" below. |
...
**`workflow_dispatch` takes an `only` input** (`all` | `cve-check` | `image-scan` | `e2e` | `e2e-shared` | `e2e-prod-backends` | `nvd-cache-refresh`) so you can run ONE job manually instead of the whole workflow. Without it a manual run fans out to every dispatch-gated job (~65 billed min), which made "just re-run the image scan" absurd on this budget. Every selector value must name a real job — the dispatch-gated jobs each carry `contains(fromJSON('["all","<self>"]'), inputs.only)`.

### NVD cache — MANUAL warmer; a cold ~70-90 min sync at release time is the ACCEPTED default

GitHub evicts an Actions cache after **7 days without a hit**, and `cve-check` runs on **tag pushes only**. So unless this repo is tagged more often than weekly, the NVD cache is **always cold at release time** and `cve-check` pays a full **~70-90 min NVD sync** (hence its `timeout-minutes: 120` — being killed mid-download is exactly what produced the never-completes-a-scan loop that started this whole rework).

**That is a deliberate, accepted trade-off, not an oversight.** A weekly `nvd-cache-refresh` cron would keep the cache warm (~2-3 min/week after a one-time ~70-90 min cold bootstrap), but the owner opted out of the weekly spend (2026-07-13). The job still exists — it is simply **manual-only**.

**To make a release fast:** dispatch the warmer first, then tag within 7 days.

```
Actions → CI → Run workflow → only: nvd-cache-refresh     # populates nvd-db-v2-<ISO-week>
# ... then push the v* tag within 7 days; cve-check restores the warm cache (~2-3 min).
```

`make nvd-refresh` → `dependency-check:update-only` refreshes the NVD DB and analyzes **nothing** — it CANNOT fail on a CVE and can never redden `main`. It saves the **same cache key + path** the `cve-check` job restores (`nvd-db-v2-<ISO-week>` @ `~/.m2/repository/org/owasp/dependency-check-data`) — **if those ever drift apart, the warmer warms a cache nobody reads and releases silently stay slow.** Skipping the warmer costs nothing but time: the release-day scan is slow, never wrong.
...
- `cve-check` runs in CI on **tag pushes + `workflow_dispatch` only** (`timeout-minutes: 120`). It used to also run on the weekly cron — that was removed 2026-07-13 because it could never succeed: the NVD H2 cache key is the ISO week and GitHub evicts a cache after 7 days without a hit, so a weekly-only job always restored a **cold** cache, spent its whole 45-min budget on a full ~365k-record NVD sync, was killed at `timeout-minutes` (reported as `cancelled`, not a concurrency cancel), and therefore never SAVED a cache — a self-sustaining loop that burned ~45 CI-minutes every Monday and never once completed a scan. **Tag-gating alone does NOT keep the cache warm** — that was the flawed first assumption; with a 7-day eviction and tags less frequent than weekly, every release is cold. And with `nvd-cache-refresh` now MANUAL-ONLY (owner opted out of the weekly warmer), **a cold ~70-90 min sync is the NORMAL release-day path, not a rare worst case** — hence the 120-min ceiling: being killed mid-download is precisely what caused the original loop. Dispatch the warmer before tagging to make it fast (see "NVD cache" above).
...
- [x] ~~**Warm the NVD cache before the next release tag**~~ — SOLVED 2026-07-13 by the `nvd-cache-refresh` job (see "NVD cache" below). No manual action needed: the first weekly cron run bootstraps the cache (a one-time ~70-90 min cold sync, `continue-on-error`, gates nothing), and every run after that is a ~2-3 min incremental update. If you tag BEFORE that first cron run, `cve-check` pays the cold sync itself — still inside its 90-min ceiling.
