...
- The NVD H2 database is cached in CI (`actions/cache`, keyed on the ISO week with prefix `nvd-db-v2-`). `actions/cache` tars `odc.mv.db`; if an NVD update is interrupted (cancelled/timeout) or the file is archived before H2 flushes, the cached DB is **truncated** and `restore-keys` re-hydrates the poison on every subsequent run — hard-failing `cve-check` with `MVStoreException: ... length -1` → `connectionPool ... is null` NPE cascade → Maven exit 2 (root cause of the failed scheduled run 27938409200, 2026-06-22). Two-part fix: (1) the `nvd-db-v2-` key prefix orphans the poisoned pre-2026-06-22 tarballs (bump to `v3-` if it ever recurs); (2) the `cve-check` Make recipe **self-heals** — on the corruption signature it runs `org.owasp:dependency-check-maven:purge` and re-runs the scan once against a fresh download (gated on the signature so a real CVE finding still fails fast; `failOnError=false` is deliberately NOT used — it would swallow real findings too).
