...
make nvd-refresh                        # Refresh the NVD DB only (update-only — no scan, CANNOT fail on a CVE). What the weekly nvd-cache-refresh CI job runs to keep the cache warm
...
| `nvd-cache-refresh` | **weekly cron** + dispatch | Keeps the NVD cache warm so tag-time `cve-check` is fast. `continue-on-error`, gates nothing. See "NVD cache" below. |
...
### NVD cache (why `cve-check` being tag-only is not enough on its own)

GitHub evicts an Actions cache after **7 days without a hit**, and `cve-check` runs on **tag pushes only**. So unless this repo is tagged more often than weekly, the NVD cache would ALWAYS be cold at release time and **every release would pay a full ~70-90 min NVD sync** — tag-gating the *gate* was right, but it left the *cache* with nobody to keep it warm.

**`nvd-cache-refresh`** (weekly cron + dispatch) is that somebody. It runs `make nvd-refresh` → `dependency-check:update-only`, which downloads/updates the NVD DB and analyzes **nothing** — so it CANNOT fail on a CVE and can never redden `main`. It saves the **same cache key + path** the `cve-check` job restores (`nvd-db-v2-<ISO-week>` @ `~/.m2/repository/org/owasp/dependency-check-data`) — **if those ever drift apart, the warmer warms a cache nobody reads and releases silently go back to cold syncs.** It's `continue-on-error` and its script path exits 0 with a `::warning::` on a transient NVD 503: the cache is an optimization, and failing the weekly run over an upstream outage is exactly the "red Monday" this rework removed. Cost: one ~70-90 min cold bootstrap on its first run, then ~2-3 min/week.
