...
- [ ] **`PUBLIC_IP` hardcoded to `localhost:8080` breaks WebSocket under LoadBalancer** — `k8s/pizza-store.yaml` sets `PUBLIC_IP=localhost:8080`, which `PizzaStore.java` echoes back via `GET /server-info`. The browser then opens `ws://localhost:8080/ws` for STOMP — works only under `kubectl port-forward svc/pizza-store 8080:80` (the README's "Bring your own cluster" path). Under `make kind-up` (cloud-provider-kind LoadBalancer at `172.18.0.x`) or any real cloud LB, the WebSocket connect fails and the Events panel stays empty even though the order fan-out completes server-side (full `ORDER_PLACED → ORDER_COMPLETED` lifecycle visible in `kubectl logs`, but `WebSocketSession[0 current WS(0)]` in pizza-store stats). Verified 2026-04-26 during the salaboy-purge screenshot regeneration — had to fall back to port-forward to capture working UI. The e2e script (`e2e/e2e-test.sh`) does not catch this because it asserts via REST + state-store reads, never the browser WS path. Possible fixes, smallest-surface first: (a) drop `publicIp` from `/server-info` and switch `app.js` to a relative `ws:///ws` URL — Spring/Tomcat picks up whichever host the browser is talking to; (b) compute `PUBLIC_IP` from the inbound `Host:` header in `PizzaStore.java` and stop relying on the env var; (c) keep the env var but document it as port-forward-only and patch the manifest at e2e time. Option (a) is the cleanest. Adding a Playwright/puppeteer browser-path assertion to `make e2e` would prevent the next regression of this class.
