...
- [x] **`PUBLIC_IP` server-info round-trip retired; WebSocket URL derived client-side** — the legacy flow `(client) GET /server-info → {publicIp: "localhost:8080"} → ws://localhost:8080/ws` only worked under `kubectl port-forward svc/pizza-store 8080:80`; under any real LoadBalancer (cloud-provider-kind on `make kind-up`, or a cloud LB) the WebSocket connect fell through and the Events panel stayed empty while the order fan-out still completed server-side. Surfaced 2026-04-26 during the salaboy-purge screenshot regeneration — had to fall back to port-forward to capture working UI. Fix: `app.js` now builds the broker URL from `window.location.host` with a `wss:` upgrade when the page is HTTPS, so it follows whichever host the browser is talking to and survives TLS-terminating LBs for free. Server-side: `/server-info` endpoint, `Info` record, and `PUBLIC_IP @Value` field removed from `PizzaStore.java`; `PUBLIC_IP` env entries dropped from `k8s/pizza-store.yaml` and `k8s-dapr-shared/apps.yaml`; `testGetServerInfo` removed from `PizzaStoreTest.java`. Follow-up — adding a Playwright/puppeteer browser-path assertion to `make e2e` would prevent the next regression of this class. (2026-04-26)
