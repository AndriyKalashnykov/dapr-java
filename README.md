...
| **nvd-cache-refresh** | weekly cron (Mon 06:00 UTC), `workflow_dispatch` | — | Keeps the NVD database cache warm. Runs `make nvd-refresh` → `dependency-check:update-only`, which refreshes the NVD DB and analyzes **nothing** — it cannot fail on a CVE and never reddens `main` (`continue-on-error`). It exists because GitHub evicts an Actions cache after 7 days without a hit while `cve-check` runs on tags only: without a warmer, every release would pay a full ~70-90 min cold NVD sync. ~2-3 min/week once warm |
